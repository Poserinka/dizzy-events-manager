...
final class Admin
{
    ...
    public function audiencePage(): void
    {
        ...
        <div class="dizzy-nl-toolbar">
            <form class="dizzy-nl-subscriber-filter"><input type="hidden" name="page" value="dizzy-newsletter-audience"><input type="search" name="s" value="<?php echo esc_attr($search); ?>" placeholder="<?php esc_attr_e('Search subscribers', 'dizzy-newsletter'); ?>"><select name="status"><option value=""><?php esc_html_e('All statuses', 'dizzy-newsletter'); ?></option><?php foreach (['subscribed','pending','unsubscribed','bounced'] as $value) : ?><option <?php selected($status, $value); ?>><?php echo esc_html(ucfirst($value)); ?></option><?php endforeach; ?></select><button class="button"><?php esc_html_e('Filter', 'dizzy-newsletter'); ?></button></form>
