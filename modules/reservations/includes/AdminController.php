...
final class AdminController
{
    ...
    {
        $parent = current_user_can('edit_posts') ? DIZZY_EVENTS_ADMIN_MENU : self::MENU;
        if ($parent === self::MENU) {
                        ...
                        <?php foreach ($rows as $row) :
                            ...
                            <article class="dizzy-reservation-card" id="dizzy-reservation-<?php echo esc_attr((string) $id); ?>" data-reservation-date="<?php echo esc_attr((string) $row['reservation_date']); ?>">
                                <header class="dizzy-reservation-card-header">
                                    <div class="dizzy-reservation-card-summary">
                                        <strong><?php echo esc_html(sprintf('%s — %s — %s', (string) $row['name'], (string) $row['email'], (string) $row['phone'])); ?></strong>
                                        <span>» <?php echo esc_html($this->formatDate((string) $row['reservation_date'])); ?> — <?php echo esc_html(substr((string) $row['reservation_time'], 0, 5)); ?> &nbsp; » <?php echo esc_html((string) $row['guests']); ?> <?php echo esc_html((int) $row['guests'] === 1 ? __('Person', 'dizzy-reservations-manager') : __('People', 'dizzy-reservations-manager')); ?></span>
                                    </div>
    ...
    private function calendar(array $rows): void
    {
        ...
        <style>
            ...
            .dizzy-reservations-list{background:transparent;border:0}.dizzy-reservations-calendar-column{box-sizing:border-box;padding:18px;background:#fff;border:1px solid #E8E8EB;border-radius:10px;box-shadow:0 2px 5px #0000000d}.dizzy-calendar-surface{background:#fff;border:1px solid #c3c4c7}
            ...
            .dizzy-reservation-cards{display:grid;gap:20px;margin-top:20px}.dizzy-reservation-card{background:#fff;border:1px solid #E8E8EB;border-radius:10px;box-shadow:0 2px 5px #0000000d;overflow:hidden}
            .dizzy-reservation-card-header{display:flex;align-items:center;justify-content:space-between;gap:20px;min-height:48px;padding:8px 10px 8px 20px;border-bottom:1px solid #e6e9ed}.dizzy-reservation-card-summary{display:grid;gap:4px;min-width:0}.dizzy-reservation-card-summary strong,.dizzy-reservation-card-summary span{overflow-wrap:anywhere}.dizzy-reservation-card-summary span{color:#475467}.dizzy-reservation-card-header form{flex:0 0 auto}.dizzy-reservation-card-header select{min-width:146px}
            .dizzy-reservation-card-content{display:grid;grid-template-columns:48px minmax(0,1fr);gap:18px;padding:20px}.dizzy-reservation-info-label{font-weight:600}.dizzy-reservation-card dl{margin:0;padding:14px 16px;border:1px solid #ccd3dc;border-radius:3px}.dizzy-reservation-card dl>div{display:grid;grid-template-columns:100px minmax(0,1fr);gap:10px;padding:4px 0}.dizzy-reservation-card dt{font-weight:600}.dizzy-reservation-card dd{margin:0;overflow-wrap:anywhere}.dizzy-reservation-card.is-calendar-hidden{display:none}.dizzy-reservation-empty{padding:20px;background:#fff;border:1px solid #dce1e7}
